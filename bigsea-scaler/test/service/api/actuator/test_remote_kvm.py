...
class Test_Remote_KVM(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.remote_kvm = Remote_KVM(self.ssh_utils)
        self.cap = 56
        self.host_ip = "vm-ip"
        self.vm_id = "vm-id"
    ...
    def test_change_cpu_quota(self):
        ...
        self.ssh_utils.run_command.assert_called_once_with(command, "root", self.host_ip)

    def test_change_cpu_quota_negative_cap_value(self):
        self.assertRaises(Exception, self.remote_kvm.change_vcpu_quota, self.host_ip, self.vm_id, -10)

    def test_change_cpu_quota_too_high_cap_value(self):
        self.assertRaises(Exception, self.remote_kvm.change_vcpu_quota, self.host_ip, self.vm_id, 100 + 20)
    ...
    def test_get_allocated_resources_success(self):
        ...
        self.ssh_utils.run_and_get_result.assert_called_once_with(command, "root", self.host_ip)
    ...
    def test_get_allocated_resources_ssh_returns_empty_string(self):
        ...
        self.assertRaises(Exception, self.remote_kvm.get_allocated_resources, self.host_ip, self.vm_id)
