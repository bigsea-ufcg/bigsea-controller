...
class Basic_Alarm:
    ...
    def __init__(self, actuator, metric_source, trigger_down, trigger_up, min_cap, max_cap, actuation_size):
    ...
    def check_application_state(self, application_id, instances, expected_time):
        ...
        job_progress = self.metric_source.get_most_recent_value(Basic_Alarm.PROGRESS_METRIC_NAME,
                                                                {"application_id":application_id})
        self.logger.log("Getting time progress")
        time_progress = self.metric_source.get_most_recent_value(Basic_Alarm.ELAPSED_TIME_METRIC_NAME,
                                                                 {"application_id":application_id})

        self.logger.log("Progress:%d|Time progress:%d" % (job_progress, time_progress))

        diff = job_progress - 100*time_progress/float(expected_time)
